"""
The code base was taken from here: https://github.com/AtsushiSakai/PythonRobotics/blob/fa1585d880ea771abd4215077eb699d194486eb4/PathPlanning/DubinsPath/dubins_path_planning.py#L206
...
def dubins_path_planning_from_origin(end_x, end_y, end_yaw, curvature,
    ...
    x_list, y_list, yaw_list, directions = generate_local_course(
        sum(lengths), lengths, best_mode, curvature, step_size, t_interpolate)

    best_cost = best_cost * t_interpolate
...
def dubins_path_planning(s_x, s_y, s_yaw, g_x, g_y, g_yaw, c, step_size=0.1, t_interpolate=1.0):
    ...
    lp_x, lp_y, lp_yaw, mode, lengths = dubins_path_planning_from_origin(
        le_xy[0], le_xy[1], le_yaw, c, step_size, t_interpolate)
...
def generate_local_course(total_length, lengths, mode, max_curvature,
                          step_size, t_interpolate):
    total_length = total_length * t_interpolate
    if total_length <= lengths[0]:
        lengths = [total_length, 0, 0]
    elif total_length - lengths[0] <= lengths[1]:
        lengths = [lengths[0], total_length - lengths[0], 0]
    else:
        lengths = [lengths[0], lengths[1], total_length - lengths[0] - lengths[1]]
